    ...
    <table cellpadding="0" cellspacing="0" class="edit">
      <tbody>
        <tr>
          ...
          <td class="input">
            <input
              :value="makeFormatNumberWithCommas(form.totalWeight)"
              disabled
              type="text"
              class="text-right"
            />
        ...
        <tr>
          ...
          <td class="label">
            ...
            <!-- FIXME: update to i18n label -->
          </td>
          <td class="input">
            <input
              :value="makeFormatNumberWithCommas(form.totalAmount)"
              disabled
              type="text"
              class="text-right"
            />
            <!-- FIXME: update to valid data field -->
          </td>
          <td class="info"></td>
        </tr>
        <tr>
...
export default {
  ...
  async fetch() {
    try {
      await Promise.all([this.getListDepartments(), this.getEmployeeList()])
    } catch (err) {
      console.error(err)
    }
  },
