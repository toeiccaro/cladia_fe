<template>
  <div class="position-relative">
    <div
      ...
      :style="{
        height:
          dataTotalMapping && dataTotalMapping.length
            ? 'calc(100% - 22px)'
            : '100%',
      }"
      @scroll="handleScroll"
    ...
    <div
      v-if="dataTotalMapping && dataTotalMapping.length"
      id="table-total"
      ref="total"
      class="table-total"
    >
      <table id="table-resize">
        <tbody>
          <tr class="row-data">
            <td
              v-for="(item, index) in dataTotalMapping"
              :key="`total-${index}`"
              :style="{
...
export default {
  ...
  computed: {
    ...
    dataTotalMapping() {
      const colsHaveTotal = Object.keys(this.tableFooter)
      return this.headersComponent.map((item) => {
        const temp = {
          key: item.key,
          value: '',
          type: 'text',
        ...
        if (colsHaveTotal.includes(item.key)) {
          temp.value = this.tableFooter[item.key]
          temp.align = 'right'
          temp.type = 'amount'
        }
        if (item.key === 'orderNO') {
