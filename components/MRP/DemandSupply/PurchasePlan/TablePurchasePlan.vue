<template>
  <div class="table__purchase-plan d-flex flex-column position-relative">
    <BaseTableDraggable
      v-if="!isLoadingTable"
      ...
      :initial-filters="payloadPurchasePlan"
      :update-filters-function="UPDATE_PAYLOAD_PURCHASE_PLAN"
      class="table__purchase-plan--body"
      @row="handleRow"
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="payloadPurchasePlan.pageSize"
      :current-page="payloadPurchasePlan.pageNo"
      :number-item="dataTable.length"
      class="table__purchase-plan--footer"
      :class="{
        'border--full': !total,
      }"
...
import { purchasePlanSchema } from '@/schemas/mrp/purchase-plan'
...
export default {
  ...
  async fetch() {
    try {
      this.loading = true
      this.UPDATE_PAYLOAD_PURCHASE_PLAN({
        language: this.lang,
      })
      await this.getData()
    } catch (err) {
  ...
  computed: {
    ...mapGetters({
      payloadPurchasePlan: 'filterSort/getPayloadPurchasePlan',
    }),
    listDataShow() {
      ...
      this.listDataShow.forEach((item) => {
        ...
        const headerItem = {
          key: this.mappingProperty(
            this.dataTable[0] || purchasePlanSchema,
            item.fieldKey
          ),
    ...
    dataTableMapping() {
      ...
      const data = this.dataTable.map((item, index) => {
        const obj = {
          index: {
            value:
              this.payloadPurchasePlan.pageSize *
                (this.payloadPurchasePlan.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
    ...
    dataTable: {
      ...
      handler(value) {
        this.listCheckbox = value.map((item) => ({
          ...
          lineID: item.lineID,
          relatedOrderNumber: item.relateOrderNO,
          moq: item.moq,
          value: false,
          unit: item.unit,
        }))
        this.listMemoChange = []
        this.SET_PURCHASE_PLAN_MEMOS([])
      },
    },
  ...
  created() {
    this.SET_PAYLOAD_PURCHASE_PLAN({
      language: this.lang,
      pageNo: 1,
      pageSize: 30,
    })
  ...
  methods: {
    ...mapMutations({
      UPDATE_PAYLOAD_PURCHASE_PLAN: 'filterSort/UPDATE_PAYLOAD_PURCHASE_PLAN',
      SET_PAYLOAD_PURCHASE_PLAN: 'filterSort/SET_PAYLOAD_PURCHASE_PLAN',
      SET_PURCHASE_PLAN_TOTAL_PAGES: 'mrp/SET_PURCHASE_PLAN_TOTAL_PAGES',
      SET_PURCHASE_PLAN_MEMOS: 'mrp/SET_PURCHASE_PLAN_MEMOS',
    }),
    async getData() {
      try {
        this.isLoadingTable = true
        const res = await api('searchPurchasePlan', this.payloadPurchasePlan)
        ...
        this.SET_PURCHASE_PLAN_TOTAL_PAGES(res.data?.tableContent?.totalPages)
      } catch (err) {
        console.error(err)
      } finally {
    ...
    async filterAndSort() {
      try {
        this.loading = true
        const fieldValue = this.payloadPurchasePlan.sortByColumn
        const payload = {
          ...this.payloadPurchasePlan,
          sortByColumn: fieldValue === 'orderNO' ? 'id' : fieldValue,
        }
        const res = await api('searchPurchasePlan', payload)
        const validResponse = res && res.status === SERVER_RESPONSE_CODE.OK
        if (validResponse) {
          this.headerTable = res.data.scolumnHides
          this.dataTable = res.data.tableContent?.content
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_PURCHASE_PLAN(filterPayload)
      this.filterAndSort()
    },
    ...
    onChangeMemo(value, item) {
      const idx = this.listMemoChange.findIndex(
        (el) => el.purchasePlanID === item.id.value
      )
      const memo = {
        purchasePlanID: item.id.value,
        memo: item.memo.value,
      }
      if (idx !== -1) {
        this.listMemoChange[idx].memo = value
        this.SET_PURCHASE_PLAN_MEMOS(cloneDeep(this.listMemoChange))
        return
      }
...
.table__purchase-plan {
  height: calc(100% - 70px);

  .table__purchase-plan--body {
    height: calc(100% - 26px);
  }

  .table__purchase-plan--footer {
    color: #000000;
