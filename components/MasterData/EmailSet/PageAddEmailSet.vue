...
export default {
  ...
  methods: {
    ...
    async handleNewSave() {
      ...
      this.emailSetData = {
        moduleID: this.emailSetData?.moduleID,
      }
    },
    ...
    async handleSave() {
      try {
        ...
        const response = await api('createBEmailSet', payload)
        const validResponse =
        ...
        if (validResponse) {
          ...
          return this.$router.push(
            this.localePath(
              `/master-data/email-set/detail?employeeID=${createdData.employeeID}&moduleID=${createdData.moduleID}`
            )
          )
        }
    ...
    removeUrlQueryParams() {
      ...
      const hasQueryParams = query.moduleID
