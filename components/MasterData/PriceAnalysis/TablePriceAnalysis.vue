...
export default {
    ...
    dataTableMapping() {
      const data = this.dataTablePrice.map((item, index) => {
        ...
        this.listDataShow.forEach((headerItem, headerIndex) => {
          ...
          if (listFieldAlginRight.includes(headerItem.fieldName)) {
            obj[headerItem.key] = {
              align: 'right',
              value: item[headerItem.key] || '',
              color: headerItem.color || '',
          ...
          const listFieldCurrency = ['PurchaseCurrency', 'SaleCurrency']
          if (listFieldCurrency.includes(headerItem.fieldName)) {
            obj[headerItem.key] = {
              value: this.mapCurrency(item[headerItem.key]) || '',
              color: headerItem.color || '',
            }
          }
        })
  ...
  methods: {
    ...
    mapCurrency(currencyID) {
      const mappedCurrency = this.currencyOptions?.find(
        (item) => item.value === Number(currencyID)
      )
      if (mappedCurrency) return mappedCurrency.text
      return ''
    },
