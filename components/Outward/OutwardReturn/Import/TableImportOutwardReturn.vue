<template>
  <div class="table__view-import position-relative">
    <BaseTableDraggable
      ...
      :initial-filters="payloadOutwardReturnImport"
      :update-filters-function="UPDATE_PAYLOAD_OUTWARD_RETURN_IMPORT"
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="payloadOutwardReturnImport.pageSize"
      :current-page="payloadOutwardReturnImport.pageNo"
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      payloadOutwardReturnImport: 'filterSort/getPayloadOutwardReturnImport',
    }),
    listDataShow() {
    ...
    dataTableMapping() {
      const data = this.dataTable.map((item, index) => {
        const obj = {
          index: {
            value:
              this.payloadOutwardReturnImport.pageSize *
                (this.payloadOutwardReturnImport.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
  ...
  destroyed() {
    this.SET_PAYLOAD_OUTWARD_RETURN_IMPORT({
      language: this.lang,
      pageNo: 1,
      pageSize: 30,
    })
  ...
  methods: {
    ...mapMutations({
      ...
      SET_PAYLOAD_OUTWARD_RETURN_IMPORT:
        'filterSort/SET_PAYLOAD_OUTWARD_RETURN_IMPORT',
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_OUTWARD_RETURN_IMPORT(filterPayload)
      this.getData()
    },
    ...
    async filterAndSort() {
      try {
        this.loading = true
        const res = await api(
          'getOutwardReturnImportList',
          this.payloadOutwardReturnImport
        )
