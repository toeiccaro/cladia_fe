<template>
  <div class="table-order d-flex flex-column position-relative">
    <BaseTableDraggable
      ...
      :initial-filters="getPayloadOutwardReturn"
      :update-filters-function="UPDATE_PAYLOAD_OUTWARD_RETURN"
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="getPayloadOutwardReturn.pageSize"
      :current-page="getPayloadOutwardReturn.pageNo"
...
export default {
  ...
  async fetch() {
    try {
      this.loading = true
      this.UPDATE_PAYLOAD_OUTWARD_RETURN({
        language: this.lang,
      })
  ...
  computed: {
    ...mapGetters({
      ...
      getPayloadOutwardReturn: 'filterSort/getPayloadOutwardReturn',
    }),
    ...
    listDataShow() {
      return (
        this.dataHeader
          ...
          // .map((item) => {
          //   if (item.fieldName === 'ArrivalAddress') {
          //     item.fieldName = 'DeliveryAddress'
          //   }
    ...
    dataTableMapping() {
      ...
      const data = this.dataTable?.map((item, index) => {
        const obj = {
          index: {
            value:
              this.getPayloadOutwardReturn.pageSize *
                (this.getPayloadOutwardReturn.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
        ...
        this.listDataShow?.forEach((headerItem) => {
          ...
          switch (headerItem.fieldName) {
            ...
            case 'OrderNO':
              ...
              obj[
                fieldKey
              ].link = `/${this.$i18n.locale}/outward/outward-return/detail?orderNo=${item.orderNO}`
              obj[fieldKey].value = item.orderNO 
              break
          }
        })
  ...
  created() {
    ...
    this.SET_PAYLOAD_OUTWARD_RETURN(payload)
  },
  methods: {
    ...mapActions('base', ['getUnitOptions']),
    ...mapMutations({
      UPDATE_PAYLOAD_OUTWARD_RETURN: 'filterSort/UPDATE_PAYLOAD_OUTWARD_RETURN',
      SET_PAYLOAD_OUTWARD_RETURN: 'filterSort/SET_PAYLOAD_OUTWARD_RETURN',
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_OUTWARD_RETURN(filterPayload)
      this.getData()
    },
    ...
    async getData() {
      try {
        ...
        const res = await api(
          'getListOutwardReturn',
          this.getPayloadOutwardReturn
        )
