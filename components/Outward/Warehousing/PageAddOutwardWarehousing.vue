<template>
  <div class="add-outward-warehousing-page">
    <ToolBar
    ...
    <BaseTableItemDetail
      ref="tableItem"
      class="add-outward-warehousing-table-details"
...
export default {
  name: 'PageAddOutwardWarehousing',
  components: {
    ToolBar,
    FormOutwardWarehousing,
    BaseTableItemDetail,
  ...
  watch: {
    'form.quantity': {
      handler() {
        this.form.amount =
          parseToNumber(this.form?.quantity) * parseToNumber(this.form?.price)
      },
      deep: true,
      immediate: true,
    },
  ...
  methods: {
    ...
    validateDetailForm() {
      ...
      if (this.availableListDetails.length === 0) {
    ...
    async handleSave() {
      try {
        ...
        const apiParams = {
          ...this.form,
          currencyID: this.form.currencyID || 0,
          unitID: this.form.unitID || 0,
        ...
        const response = await api('addOutwardWarehousing', apiParams)
        ...
        this.$router.push({
          path: this.localePath(`/outward/outward-warehousing/detail`),
          query: {
    ...
    handleImportData(data) {
      ...
      this.form.currencyID = data.currencyID?.value
      this.form.price = data.price?.value
      this.form.amount = (data.blance?.value || 0) * data.price?.value
      this.form.wpoId = data.wpoId?.value
      this.form.quantity = data.blance?.value || 0
      this.form.wpono = data.orderNo?.value
      this.dataTable = []
