<template>
  <div class="table-outward-warehousing d-flex flex-column position-relative">
    <BaseTableDraggable
      v-if="!isLoadingTable"
      :header="headerMapping"
      :data="dataTableMapping"
      :data-total="dataTotalMapping"
      class="table-outward-warehousing--body"
      :initial-filters="payloadOutwardWarehousing"
      :update-filters-function="UPDATE_PAYLOAD_OUTWARD_WAREHOUSING"
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="payloadOutwardWarehousing.pageSize"
      :current-page="payloadOutwardWarehousing.pageNo"
      :number-item="dataTable.length"
      class="table-outward-warehousing--footer"
...
export default {
  ...
  async fetch() {
    try {
      this.loading = true
      this.UPDATE_PAYLOAD_OUTWARD_WAREHOUSING({
        language: this.lang,
      })
      await this.getData()
    } catch (err) {
  ...
  computed: {
    ...
    dataTableMapping() {
      const data = this.dataTable.map((item, index) => {
        const obj = {
          index: {
            value:
              this.payloadOutwardWarehousing.pageSize *
                (this.payloadOutwardWarehousing.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
        ...
        this.listDataShow.forEach((headerItem, headerIndex) => {
          ...
          const alignRight = ['Quantity', 'Price', 'Amount', 'SubQuantity']
          ...
          if (headerItem.fieldName === 'OrderNO') {
            ...
            obj[
              fieldKey
            ].link = `/${this.$i18n.locale}/outward/outward-warehousing/detail?orderNo=${item.orderNo}`
          }
        })
        return obj
      })
    ...
    headerMapping() {
      const listNumberField = ['Quantity', 'Price', 'Amount', 'SubQuantity']
      const header = [
        {
          key: 'index',
          name: '',
  ...
  created() {
    ...
    this.SET_PAYLOAD_OUTWARD_WAREHOUSING(payload)
  },
  methods: {
    ...mapMutations({
      UPDATE_PAYLOAD_OUTWARD_WAREHOUSING:
        'filterSort/UPDATE_PAYLOAD_OUTWARD_WAREHOUSING',
      SET_PAYLOAD_OUTWARD_WAREHOUSING:
        'filterSort/SET_PAYLOAD_OUTWARD_WAREHOUSING',
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_OUTWARD_WAREHOUSING(filterPayload)
      this.getData()
    },
    ...
    handleActiveRow(payload) {
      this.$emit('handleActiveRow', payload.item || {})
    },
    changeLayout(data) {
...
.table-outward-warehousing {
  // border: 1px solid #5180d8;
  height: calc(100% - 70px);
  .table-outward-warehousing--body {
    height: calc(100% - 26px);
  }
  .table-outward-warehousing--footer {
    color: #000000;
