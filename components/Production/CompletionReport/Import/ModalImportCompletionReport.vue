  ...
  <div v-if="showModal">
    <div class="modal-container">
      ...
      <div class="modal-content">
        ...
        />
        <TableImportCompletionReport
          ref="importProductionCompletionReport"
          :use-checkbox="true"
          :data-form="form"
    ...
    <BaseSetColumn
      ref="modalSetColumn"
      full-width
      @on-close="$refs.importProductionCompletionReport.refresh()"
    ></BaseSetColumn>
  </div>
  <div
    v-else-if="loading"
...
<script>
import TableImportCompletionReport from './TableImportCompletionReport.vue'
import api from '@/api/api'
import ToolBar from '@/components/UI/ToolBar.vue'
import BaseSetColumn from '~/components/UI/BaseSetColumn.vue'
export default {
  name: 'ModalImportProductionCompletionReport',
  components: {
    ToolBar,
    BaseSetColumn,
    TableImportCompletionReport,
  ...
  methods: {
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.importProductionCompletionReport?.refresh()
      }
    },
    async handleAction(key) {
      if (key === 'refresh') {
        this.$refs.importProductionCompletionReport?.checkAll(false)
        return await this.$refs.importProductionCompletionReport?.refresh()
      }
      if (key === 'saveLayout') {
    ...
    handleImportData() {
      const availableItems =
        this.$refs.importProductionCompletionReport?.listDataSelected || []
      const hasSelectedDetails = availableItems.length > 0
      if (!hasSelectedDetails) {
        return window.alert(this.$t('msg_NoSelected_0'))
      }

      const finalSelectedItems = availableItems.map((item) =>
        Object.assign(item, { quantity: item.quantity || item.soqty })
      )

      this.$emit('update-details', finalSelectedItems)
