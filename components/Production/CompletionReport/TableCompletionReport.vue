<template>
  <div class="table-order d-flex flex-column position-relative">
    <BaseTableDraggable
      ...
      :initial-filters="getPayloadCompletionReport"
      :update-filters-function="UPDATE_PAYLOAD_COMPLETION_REPORT"
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="getPayloadCompletionReport.pageSize"
      :current-page="getPayloadCompletionReport.pageNo"
...
export default {
  ...
  data() {
    return {
      ...
      listFieldNameIgnore: [
        ...
        'DeliveryDate',
        'DeliveryLT',
      ], // ignore label form list label get from api
    }
  },
  async fetch() {
    try {
      this.loading = true
      this.UPDATE_PAYLOAD_COMPLETION_REPORT({
        language: this.lang,
      })
  ...
  computed: {
    ...
    dataTableMapping() {
      ...
      const data = this.dataTable?.map((item, index) => {
        const obj = {
          index: {
            value:
              this.getPayloadCompletionReport.pageSize *
                (this.getPayloadCompletionReport.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
          ...
          switch (headerItem.fieldName) {
            ...
            case 'OrderNO':
              obj.orderNo = {
                ...
                link: `/${this.$i18n.locale}/production/completion-report/detail?orderNo=${item.orderNo}`,
              }
              break
          }
        })
    ...
    headerMapping() {
      ...
      const getHeaderItem = (item) => {
        ...
        const listHeaderSelect = ['IsCheck', 'IsBad', 'IsPrintedLabel']
  ...
  created() {
    ...
    this.SET_PAYLOAD_COMPLETION_REPORT(payload)
  },
  methods: {
    ...mapActions('base', ['getUnitOptions']),
    ...mapMutations({
      UPDATE_PAYLOAD_COMPLETION_REPORT:
        'filterSort/UPDATE_PAYLOAD_COMPLETION_REPORT',
      SET_PAYLOAD_COMPLETION_REPORT: 'filterSort/SET_PAYLOAD_COMPLETION_REPORT',
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_COMPLETION_REPORT(filterPayload)
      this.getData()
    },
