<template>
  <div class="table-order d-flex flex-column position-relative">
    <BaseTableDraggable
      ...
      :initial-filters="getPayloadMaterialDelivery"
      :update-filters-function="UPDATE_PAYLOAD_MATERIAL_DELIVERY"
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="getPayloadMaterialDelivery.pageSize"
      :current-page="getPayloadMaterialDelivery.pageNo"
...
export default {
  ...
  data() {
    return {
      ...
      listFieldNameIgnore: ['Department', 'LineID', 'SOLineID'],
    }
  },
  async fetch() {
    try {
      this.loading = true
      this.UPDATE_PAYLOAD_MATERIAL_DELIVERY({
        language: this.lang,
      })
  ...
  computed: {
    ...mapGetters({
      ...
      getPayloadMaterialDelivery: 'filterSort/getPayloadMaterialDelivery',
    }),
    ...
    dataTableMapping() {
      const listAlignRightFields = ['Quantity', 'SubQuantity', 'DeliveredQuantity', 'BalanceQty']

      const data = this.dataTable?.map((item, index) => {
        const obj = {
          index: {
            value:
              this.getPayloadMaterialDelivery.pageSize *
                (this.getPayloadMaterialDelivery.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
        ...
        this.listDataShow?.forEach((headerItem) => {
          ...
          if (headerItem.fieldName === 'LT') {
            const propName = fieldKey.toLowerCase()
            obj[propName] = {
          ...
          switch (headerItem.fieldName) {
            ...
            case 'OrderNO':
              obj.orderNo = {
                ...
                link: `/${this.$i18n.locale}/production/material-delivery/detail?orderNo=${item.orderNo}`,
              }
              break
          }
        })
    ...
    headerMapping() {
      ...
      const listNumberFields = ['Quantity', 'SubQuantity', 'LT']
      const getHeaderItem = (item) => {
        const maxLength = listNumberFields.includes(item.fieldName) ? '30' : '256'
        const itemKey = item.fieldKey === 'LT' ? item.fieldKey.toLowerCase() : item.fieldKey
        const headerItem = {
          key: itemKey,
  ...
  created() {
    ...
    this.SET_PAYLOAD_MATERIAL_DELIVERY(payload)
  },
  methods: {
    ...mapActions('base', ['getUnitOptions']),
    ...mapMutations({
      UPDATE_PAYLOAD_MATERIAL_DELIVERY:
        'filterSort/UPDATE_PAYLOAD_MATERIAL_DELIVERY',
      SET_PAYLOAD_MATERIAL_DELIVERY: 'filterSort/SET_PAYLOAD_MATERIAL_DELIVERY',
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_MATERIAL_DELIVERY(filterPayload)
      this.getData()
    },
    ...
    async getData() {
      try {
        ...
        const res = await api(
          'getMaterialDelivery',
          this.getPayloadMaterialDelivery
        )
