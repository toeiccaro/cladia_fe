<template>
  <div class="table__view-import position-relative">
    <BaseTableDraggable
      ...
      :initial-filters="getPayloadProductionOrderImport"
      :update-filters-function="UPDATE_PAYLOAD_PRODUCTION_ORDER_IMPORT"
      ...
      <slot v-for="(item, index) in dataTable" :slot="'checkbox-' + index">
        ...
        >
          <input
            ...
            @change="
              (event) =>
                onCheckImportData({ index, isChecked: event.target.checked })
            "
          />
        </div>
      </slot>
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="getPayloadProductionOrderImport.pageSize"
      :current-page="getPayloadProductionOrderImport.pageNo"
...
export default {
  ...
  computed: {
    ...mapGetters({
      getPayloadProductionOrderImport:
        'filterSort/getPayloadProductionOrderImport',
    }),
    listDataShow() {
    ...
    dataTableMapping() {
      const data = this.dataTable.map((item, index) => {
        const obj = {
          index: {
            value:
              this.getPayloadProductionOrderImport.pageSize *
                (this.getPayloadProductionOrderImport.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
  ...
  destroyed() {
    this.SET_PAYLOAD_PRODUCTION_ORDER_IMPORT({
      language: this.lang,
      pageNo: 1,
      pageSize: 30,
    })
  ...
  methods: {
    ...mapMutations({
      ...
      SET_PAYLOAD_PRODUCTION_ORDER_IMPORT:
        'filterSort/SET_PAYLOAD_PRODUCTION_ORDER_IMPORT',
      SET_DATA_COLUMN_HIDE: 'SET_DATA_COLUMN_HIDE',
    }),

    onCheckImportData({ index, isChecked }) {
      this.listCheckbox = this.listCheckbox.map((item) => ({
        ...item,
        value: false,
      }))
      this.listCheckbox[index].value = isChecked
    },
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_PRODUCTION_ORDER_IMPORT(filterPayload)
      this.getData()
    },
    ...
    async getData() {
      try {
        ...
        const res = await api(
          ...
          this.getPayloadProductionOrderImport
        )
