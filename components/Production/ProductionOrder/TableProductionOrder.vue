<template>
  <div class="table-order d-flex flex-column position-relative">
    <BaseTableDraggable
      ...
      :initial-filters="getPayloadProductionOrder"
      :update-filters-function="UPDATE_PAYLOAD_PRODUCTION_ORDER"
    ...
    <BasePagination
      v-if="!isLoadingTable"
      :total="total"
      :per-page="getPayloadProductionOrder.pageSize"
      :current-page="getPayloadProductionOrder.pageNo"
...
export default {
  ...
  async fetch() {
    try {
      this.loading = true
      this.UPDATE_PAYLOAD_PRODUCTION_ORDER({
        language: this.lang,
      })
      await Promise.all([
        this.getData(),
  ...
  computed: {
    ...mapGetters({
      ...
      operationTypeOptions: 'base/getOperationTypeOptions',
    }),
    ...
    dataTotalMapping() {
      const propertyMapping = {
        ...
        quantity: {
          value: this.calculateSum('productionQuantity'),
          align: 'right',
          type: 'amount',
        },
        subQuantity: {
          value: this.calculateSum('subQuantity'),
          align: 'right',
          type: 'amount',
        },
        amount: {
    ...
    dataTableMapping() {
      ...
      const data = this.dataTable?.map((item, index) => {
        const obj = {
          index: {
            value:
              this.getPayloadProductionOrder.pageSize *
                (this.getPayloadProductionOrder.pageNo - 1) +
              index +
              1,
            align: 'center',
          },
        ...
        this.listDataShow?.forEach((headerItem) => {
          ...
          switch (headerItem.fieldName) {
            ...
            case 'OperType':
              obj.operationType = {
                value: this.mapOperationType(item.operationTypeID),
              }
              obj[fieldKey].align = 'left'
              break
            case 'OrderDate':
            ...
            case 'OrderNO':
              obj.orderNO = {
                ...
                link: `/${this.$i18n.locale}/production/production-order/detail?orderNo=${item.orderNO}`,
              }
              break
          }
        })
    ...
    headerMapping() {
      const listNumberFields = ['Quantity', 'ProductionQuantity']
      const header = [
        {
          key: 'index',
          name: '',
      ...
      const getHeaderItem = (item) => {
        const fieldNameSelect = ['IsCheck', 'IsFinished']
        const maxLength = listNumberFields.includes(item.fieldName)
          ? '30'
          : '256'
        const headerItem = {
          key: item.fieldKey,
          name: this.$t(`lbl_${item.fieldName}_0`),
          filter: fieldNameSelect.includes(item.fieldName) ? 'select' : 'input',
  ...
  created() {
    ...
    this.SET_PAYLOAD_PRODUCTION_ORDER(payload)
  },
  methods: {
    ...mapActions('base', ['getUnitOptions', 'getOperationTypeOptionsFromAPI']),
    ...mapMutations({
      UPDATE_PAYLOAD_PRODUCTION_ORDER:
        'filterSort/UPDATE_PAYLOAD_PRODUCTION_ORDER',
      SET_PAYLOAD_PRODUCTION_ORDER: 'filterSort/SET_PAYLOAD_PRODUCTION_ORDER',
    ...
    changePerPage(value) {
      ...
      this.UPDATE_PAYLOAD_PRODUCTION_ORDER(filterPayload)
      this.getData()
    },
    ...
    mapOperationType(operationTypeId) {
      return (
        this.operationTypeOptions.find(
          (item) => operationTypeId && item.value === operationTypeId
        )?.text || ''
      )
    },
  },
