<template>
  <div v-if="showModal">
    ...
    <BaseSetColumn
      ...
      full-width
      @on-close="$refs.importPurchaseArrival.refresh()"
    ></BaseSetColumn>
  </div>
  <div
    v-else-if="loading"
...
<script>
import TableImportArrival from './TableImportArrival.vue'
import api from '@/api/api'
import ToolBar from '@/components/UI/ToolBar.vue'
import BaseSetColumn from '~/components/UI/BaseSetColumn.vue'
export default {
  name: 'ModalImportPurchaseArrival',
  components: {
    ToolBar,
    BaseSetColumn,
    TableImportArrival,
  ...
  methods: {
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.importPurchaseArrival?.refresh()
      }
    },
    async handleAction(key) {
      if (key === 'refresh') {
        this.$refs.importPurchaseArrival?.checkAll(false)
        this.$refs.importPurchaseArrival.queryPayload = {
          pageNo: 1,
          pageSize: 30,
          localLanguage: this.$i18n.locale,
        }
        return await this.$refs.importPurchaseArrival?.refresh()
      }
      if (key === 'saveLayout') {
      ...
      if (key === 'okSaveDetail') {
        const availableItems =
          this.$refs.importPurchaseArrival?.listCheckbox || []
        ...
        const finalSelectedItems = selectedItems
          .filter((_el) => delete _el.memoDTL)
          .map((item) =>
            Object.assign(item, {
              quantity: item.blance || item.poqty,
