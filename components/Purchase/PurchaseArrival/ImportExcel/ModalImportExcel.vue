  ...
  methods: {
    ...
    async mode() {
      try {
        ...
        const response = await api('getPurchaseArrivalMode', payload)
        const validResponse =
    ...
    async save() {
      try {
        const acceptFileTypes = ['xlsx', 'xls', 'xlsm']
        if (!acceptFileTypes.includes(this.fileType)) {
          return window.alert('エクセル形式のみインポート可能: xlsx, xls, xlsm')
        }

        const formData = new FormData()
        formData.append('file', this.file, this.fileName)

        const res = await api('readPurchaseArrivalExcel', formData)
        const validResponse = res && res.status === SERVER_RESPONSE_CODE.OK

        if (validResponse) {
          this.$emit('update-details', res.data)
          this.fileName = ''
          this.$refs.fileUpload.value = null
          this.showModal = false
          return window.alert(this.$t('msg_Successfully_0'))
        }
