  ...
  <div :key="refreshKey" class="add-form-enterprise">
    <table cellpadding="0" cellspacing="0" class="edit">
      <tbody>
        ...
        <tr class="tr-2">
          <td class="label">
            <span id="CustomerCode">{{ $t('lbl_Memo_0') }}</span>
          </td>
          <td rows="1" colspan="4" class="input position-relative">
            <input v-model="purchaseInvoiceData.memo" type="text" />
          </td>
          <td class="info"></td>
          <td class="label">
        ...
        <tr class="tr-4">
          ...
          <td class="input">
            <input v-model="purchaseInvoiceData.checker" disabled />
          </td>
          <td class="info"></td>
        </tr>
...
export default {
  ...
  async fetch() {
    try {
      this.purchaseInvoiceData = Object.assign(
        {},
        this.defaulPurchaseInvoiceData
      )
      await Promise.all([
  ...
  watch: {
    purchaseInvoiceData: {
      ...
      immediate: true,
      handler(data) {
        this.$emit('update-purchase-invoice-data', Object.assign({}, data))
      },
    },
    getCurrency: {
      deep: true,
      immediate: true,
      handler(data) {
        this.defaulPurchaseInvoiceData.currencyID = data
        this.purchaseInvoiceData.currencyID = data
      },
    },
  },
  methods: {
    ...
    updateSalePriceData({ path, data }) {
      this.purchaseInvoiceData[path] = data
      this.$forceUpdate()
    },
