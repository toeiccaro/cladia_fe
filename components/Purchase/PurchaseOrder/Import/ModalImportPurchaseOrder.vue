<template>
  <div v-if="showModal">
    ...
    <BaseSetColumn
      ...
      full-width
      @on-close="$refs.importPurchaseOrder.refresh()"
    ></BaseSetColumn>
  </div>
  <div
    v-else-if="loading"
  ...
  methods: {
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.importPurchaseOrder?.refresh()
      }
    },
    async handleAction(key) {
      if (key === 'refresh') {
        this.$refs.importPurchaseOrder?.checkAll(false)
        this.$refs.importPurchaseOrder.queryPayload = {
          pageNo: 1,
          pageSize: 30,
          localLanguage: this.$i18n.locale,
        }
        return await this.$refs.importPurchaseOrder?.refresh()
      }
      if (key === 'saveLayout') {
      ...
      if (key === 'okSaveDetail') {
        const availableItems =
          this.$refs.importPurchaseOrder?.listDataSelected || []
        ...
        const finalSelectedCustomers = availableItems.map((item) =>
