<template>
  <div v-if="showModal">
    ...
    <BaseSetColumn
      ...
      full-width
      @on-close="$refs.importSaleReturn.refresh()"
    ></BaseSetColumn>
  </div>
  <div
    v-else-if="loading"
...
<script>
import TableImportSaleReturn from './TableImportSaleReturn.vue'
import api from '@/api/api'
import ToolBar from '@/components/UI/ToolBar.vue'
import BaseSetColumn from '~/components/UI/BaseSetColumn.vue'
export default {
  name: 'ModalImportSaleReturn',
  components: {
    ToolBar,
    BaseSetColumn,
    TableImportSaleReturn,
  ...
  methods: {
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.importSaleReturn?.refresh()
      }
    },
    async handleAction(key) {
      if (key === 'refresh') {
        this.$refs.importSaleReturn?.checkAll(false)
        this.$refs.importSaleReturn.queryPayload = {
          pageNo: 1,
          pageSize: 30,
          localLanguage: this.$i18n.locale,
        }
        return await this.$refs.importSaleReturn?.refresh()
      }
      if (key === 'saveLayout') {
      ...
      if (key === 'okSaveDetail') {
        const availableItems = this.$refs.importSaleReturn?.listCheckbox || []
        ...
        const finalSelectedCustomers = selectedItems.map((item) =>
          Object.assign(item, {
            rtQuantity: item.blance,
          })
        )

        this.$emit('update-sale-return-details', finalSelectedCustomers)
