...
export default {
  ...
  methods: {
    ...
    async getAllTextLanguages() {
      try {
        this.loading = true
        const response = await api('getAllTextLanguages', this.sortFormOptional)
        this.headers = response?.data?.scolumnHides

        const mappedDataTable = this.mapDataTable(
          response?.data?.tableContent?.content
        )
        this.listTextLanguage = mappedDataTable
        this.totalElements = response?.data?.tableContent?.totalElements
        this.loading = false
      } catch (error) {
      } finally {
        this.loading = false
    ...
    async handleFilterAndSort({ sortParams, filterParams }, type) {
      try {
        ...
        const finalFilterParams = { languageServices: [] }
        for (const [property, value] of Object.entries(filterParams)) {
          if (property === 'keyCode') {
            ...
            finalFilterParams.languageServices.push({
              languageName: value,
              language: languageHeader || property,
            })
          }
        }
        ...
        if (
          !finalFilterParams ||
          finalFilterParams.languageServices.length === 0
        ) {
        ...
        const { data } = await api('getAllTextLanguages', searchFormOptional)
        if (data) {
          const mappedDataTable = this.mapDataTable(data.tableContent?.content)
          this.listTextLanguage = mappedDataTable
          this.totalElements = data.tableContent?.totalElements
        }
      } catch (err) {
        console.error(err)
      } finally {
    ...
    mapDataTable(rawDataContent) {
      ...
      const tableMappingData = ['keyCode', 'menuID', 'id', ...this.headers]
      rawDataContent.forEach((item) => {
        const dataItem = {}

        tableMappingData.forEach((fieldName, index) => {
          dataItem[fieldName] = item[index]
