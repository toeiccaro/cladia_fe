...
import Table from '@/components/Finance/ReceivableAnnualTable/Table.vue'
...
export default {
  ...
  methods: {
    ...
    functionReload() {
      ...
      return this.$refs.tableReceivableAnnual.refresh()
    },
    exportByPage() {
    ...
    changeActiveToolBar(key) {
      if (key === 'addOrder') {
        ...
        this.$refs.tableReceivableAnnual.query();
      } 
    },
    handleDetailId(data) {
      this.currentId = data
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.tableReceivableAnnual.refresh()
        this.loading = false
      }
    },
    async handleExportExcel(sortFormOptional) {
      this.loading = true
      const response = await api('exportReceiveQuery', sortFormOptional)
      this.loading = false
