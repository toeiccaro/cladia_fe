...
import Table from '@/components/Finance/DepositWithdraw/DepositWithdrawTable.vue'
import api from '@/api/api'
import { downloadFileExcel } from '@/utils/utils'
import BaseSetColumn from '~/components/UI/BaseSetColumn.vue'
import BaseSettingInitBlance from '~/components/UI/BaseSettingInitBlanceModal.vue'

export default {
  components: { ToolBar, Table, BaseSetColumn ,BaseSettingInitBlance},
  middleware: ['authenticated'],
  data() {
    return {
      listToolBars: [
        {
          key: 'queryTableDepositWithdraw',
  ...
  methods: {
    functionReload() {
      ...
      return this.$refs.tableDepositWithdraw.refresh()
    },
    exportByPage() {
    ...
    exportAll() {
      ...
      const sortFilter = this.payloadDepositWithdrawQuery
    ...
    changeActiveToolBar(key) {
      if (key === 'queryTableDepositWithdraw') {
        this.functionReload()
      ...
      }else if (key === 'setBlance') {
        this.$refs.modalSetBlance.showModal = true
      }
    },
    handleDetailId(data) {
      this.currentId = data
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.tableDepositWithdraw.refresh()
        this.loading = false
      }
    },
    async handleExportExcel(sortFormOptional) {
      this.loading = true
      const response = await api('exportDepositWithdrawTable', sortFormOptional)
      this.loading = false
