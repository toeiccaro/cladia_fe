...
import TableProductPlan from '@/components/MRP/DemandSupply/ProductPlan/TableProductPlan.vue'
import BaseValidateMessage from '@/components/UI/BaseValidateMessage'

export default {
  components: { ToolBar, TableProductPlan, BaseValidateMessage },
  data() {
    return {
      loading: false,
      listToolBars: [
  ...
  computed: {
    ...mapGetters({
      ...
      productPlanTotalPages: 'mrp/getProductPlanTotalPages',
      productPlanMemos: 'mrp/getProductPlanMemos',
    }),
    labelMapping() {
      return {
    ...
    listToolBarsCheckAuthority() {
      return this.listToolBars.map((item) => {
        if (item.key === TOOLBAR_KEYS.EXPORT) {
          const hasManyPages = this.productPlanTotalPages > 1
  ...
  methods: {
    ...
    handleCreateOrder() {
      ...
      const listCheckbox = this.$refs.tableProductPlan?.listCheckbox || []
      ...
      const hasDiffItems = uniqBy(checkedRecords, 'itemID').length > 1

      if (!hasDiffItems) {
        return this.validateAndGenerateOrder(checkedRecords, payload)
      }

      const confirmDiffItems = window.confirm(this.$t('msg_ConfirmDiffItems_0'))
      if (!confirmDiffItems) return
    ...
    async handleSave() {
      try {
        ...
        const hasInvalidMemo = this.productPlanMemos.some(
        ...
        if (
          ...
          this.$refs.tableProductPlan.filterAndSort()
        }
      } catch (err) {
        console.log(err)
      } finally {
    ...
    exportAll() {
      ...
      const sortFilter = this.payloadProductPlan
    ...
    async handleExportExcel(payload) {
      const response = await api('exportExcelWorkPlans', payload)
    ...
    async updateMemo() {
      try {
        const res = await api('updateWorkPlans', this.productPlanMemos)
        return res.status
      } catch (err) {
        console.log(err)
      }
    ...
    async onGenerateOrder(payload) {
      ...
      try {
        ...
        params.workPlanDetail = params.workPlanDetail.map((item) => {
          delete item.value
          return item
        })
        const hasDecimalPieces = this.hasDecimalPieces(
          params.workPlanDetail,
          params.isUsingMoq
        )
        ...
        const res = await api('generateWorkOrder', params)
        if (
        ...
        if (res.status === SERVER_RESPONSE_CODE.OK) {
          ...
          this.SET_GENERATED_PRODUCTION_ORDER({ ...data, confirmRoundUpQty })
