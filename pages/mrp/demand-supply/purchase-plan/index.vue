...
import TablePurchasePlan from '@/components/MRP/DemandSupply/PurchasePlan/TablePurchasePlan.vue'
import BaseValidateMessage from '@/components/UI/BaseValidateMessage'

export default {
  components: { ToolBar, TablePurchasePlan, BaseValidateMessage },
  data() {
    return {
      loading: false,
      listToolBars: [
  ...
  computed: {
    ...mapGetters({
      ...
      purchasePlanTotalPages: 'mrp/getPurchasePlanTotalPages',
      purchasePlanMemos: 'mrp/getPurchasePlanMemos',
    }),
    labelMapping() {
      return {
    ...
    listToolBarsCheckAuthority() {
      return this.listToolBars.map((item) => {
        if (item.key === TOOLBAR_KEYS.EXPORT) {
          const hasManyPages = this.purchasePlanTotalPages > 1
  ...
  methods: {
    ...
    handleCreateOrder() {
      ...
      const listCheckbox = this.$refs.tablePurchasePlan?.listCheckbox || []
      ...
      const payload = {
        ...
        purchasePlanDetail: checkedRecords,
      }
      ...
      const confirmDiffSuppliers = window.confirm(
        this.$t('msg_ConfirmDiffSuppliers_0')
      )
    ...
    async handleSave() {
      try {
        ...
        const hasInvalidMemo = this.purchasePlanMemos.some(
        ...
        if (
          ...
          this.$refs.tablePurchasePlan.filterAndSort()
        }
      } catch (err) {
        console.log(err)
      } finally {
    ...
    exportAll() {
      ...
      const sortFilter = this.payloadPurchasePlan
    ...
    async handleExportExcel(payload) {
      const response = await api('exportExcelPurchasePlan', payload)
    ...
    async updateMemo() {
      try {
        const res = await api('updatePurchasePlan', this.purchasePlanMemos)
        return res.status
      } catch (err) {
        console.log(err)
      }
    ...
    async onGenerateOrder(payload) {
      ...
      try {
        ...
        params.purchasePlanDetail = params.purchasePlanDetail.map((item) => {
          delete item.value
          return item
        })

        const hasDecimalPieces = this.hasDecimalPieces(
          params.purchasePlanDetail,
          params.isUsingMoq
        )
        ...
        const res = await api('generatePurchaseOrder', params)
        if (
        ...
        if (res.status === SERVER_RESPONSE_CODE.OK) {
          const mappingPODetails = Promise.all(
            res.data.listDetail.map(async (item) => {
              const params = {
          ...
          const listDetail = await mappingPODetails
          this.SET_GENERATED_PURCHASE_ORDER({ ...res.data, listDetail })
