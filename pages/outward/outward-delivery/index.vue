...
import TableOutwardDelivery from '@/components/Outward/Delivery/TableOutwardDelivery.vue'
...
export default {
  components: { ToolBar, TableOutwardDelivery, BaseSetColumn, BaseLoading },
  ...
  methods: {
    ...
    exportAll() {
      ...
      const sortFilter = this.payloadOutwardDelivery
    ...
    handleEdit() {
      this.currentId &&
        this.$router.push({
          path: `/${this.$i18n.locale}/outward/outward-delivery/detail?orderNo=${this.currentId}`,
        })
    },
    handleSet() {
    ...
    async handleExportExcel(sortFormOptional) {
      try {
        this.loading = true
        const response = await api('exportOutwardDelivery', sortFormOptional)
