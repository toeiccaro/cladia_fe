...
import TableOutwardReturn from '@/components/Outward/OutwardReturn/TableOutwardReturn.vue'
...
export default {
  components: { ToolBar, TableOutwardReturn, BaseSetColumn, BaseLoading },
  ...
  methods: {
    ...
    exportAll() {
      ...
      const sortFilter = this.getPayloadOutwardReturn
    ...
    changeActiveToolBar(key) {
      if (key === 'add') {
        ...
        this.currentId &&
          this.$router.push({
            path: `/${this.$i18n.locale}/outward/outward-return/detail?orderNo=${this.currentId}`,
          })
      } else if (key === 'set') {
    ...
    async handleExportExcel(sortFormOptional) {
      this.loading = true
      const response = await api('exportOutwardReturn', sortFormOptional)
      this.loading = false
