...
import TableOutwardInvoice from '@/components/Outward/OutwardInvoice/TableOutwardInvoice.vue'
...
export default {
  components: { ToolBar, TableOutwardInvoice, BaseSetColumn, BaseLoading },
  ...
  methods: {
    functionReload() {
      ...
      return this.$refs.tableOutwardInvoice.refresh()
    },
    exportByPage() {
    ...
    exportAll() {
      ...
      const sortFilter = this.getPayloadOutwardInvoice
    ...
    changeActiveToolBar(key) {
      if (key === 'add') {
        ...
        this.currentId &&
          this.$router.push({
            path: `/${this.$i18n.locale}/outward/wpo-invoice/detail?orderNo=${this.currentId}`,
          })
      } else if (key === 'set') {
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.tableOutwardInvoice.refresh()
        this.loading = false
      }
    },
    async handleExportExcel(sortFormOptional) {
      this.loading = true
      const response = await api('exportExcelOutwardInvoice', sortFormOptional)
      this.loading = false
