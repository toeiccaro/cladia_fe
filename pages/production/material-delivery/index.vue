<template>
  <div>
    ...
    ></ToolBar>
    <TableMaterialDelivery
      ref="tableMaterialDelivery"
...
import TableMaterialDelivery from '@/components/Production/MaterialDelivery/TableMaterialDelivery.vue'
...
export default {
  components: { ToolBar, TableMaterialDelivery, BaseSetColumn, BaseLoading },
  ...
  methods: {
    functionReload() {
      ...
      return this.$refs.tableMaterialDelivery.refresh()
    },
    exportByPage() {
    ...
    exportAll() {
      ...
      const sortFilter = this.getPayloadMaterialDelivery
    ...
    changeActiveToolBar(key) {
      if (key === 'add') {
        ...
        this.currentId &&
          this.$router.push({
            path: `/${this.$i18n.locale}/production/material-delivery/detail?orderNo=${this.currentId}`,
          })
      } else if (key === 'set') {
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.tableMaterialDelivery.refresh()
        this.loading = false
      }
    },
    async handleExportExcel(sortFormOptional) {
      this.loading = true
      const response = await api('exportExcelMaterialDelivery', sortFormOptional)
      this.loading = false
