...
import TableDNBrowse from '@/components/Sale/DNBrowse/TableDNBrowse.vue'
import api from '@/api/api'
import { downloadFileExcel } from '@/utils/utils'
import BaseSetColumn from '~/components/UI/BaseSetColumn.vue'

export default {
  components: { ToolBar, TableDNBrowse, BaseSetColumn },
  ...
  methods: {
    functionReload() {
      ...
      return this.$refs.tablDNBrowse.refresh()
    },
    async exportByPage() {
    ...
    async exportAll() {
      ...
      const sortFilter = this.payloadSaleDNBrowse
    ...
    async handleSaveLayout() {
      ...
      if (confirm) {
        ...
        this.$refs.tablDNBrowse.refresh()
        this.loading = false
      }
    },
    async handleExportExcel(queryPayload) {
      this.loading = true
      const response = await api('exportDNBrowse', queryPayload)
      this.loading = false
