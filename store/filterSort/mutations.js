...
export default {
  ...
  UPDATE_PAYLOAD_PAY_BROWSE(state, data) {
    state.payloadPayBrowse = {
      ...state.payloadPayBrowse,
      ...data,
    };

    for (const prop in state.payloadPayBrowse) {
      isEmptyValue(state.payloadPayBrowse[prop]) &&
        delete state.payloadPayBrowse[prop];
    }
  },
  SET_PAYLOAD_PAY_BROWSE(state, data) {
